...
**例** 全体收敛的实数数列所组成的集合$l^\infty$是$\boldsymbol R$上的线性空间。其中，对于$\lbrace a_n\rbrace\in l^\infty$和$\lbrace b_n\rbrace\in l^\infty$加法定义为$\lbrace a_n + b_n\rbrace$，$x\in F$，数乘定义为$\lbracexa_n\rbrace$。
